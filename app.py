...
with st.sidebar:
    ...
    if "Hipsometria" in modo_analise:
        ...
        st.markdown("🏢 **Classes de Declividade:**<br>🟢 Plano (0%) → 🟣 Crítico (+20%)", unsafe_allow_html=True)
...
# ==========================================
# 2. MOTOR DE DADOS (ETL COM DEGRADÊ LÓGICO)
# ==========================================
...
def carregar_dados_completos():
    ...
    with st.spinner("🌍 Mapeando relevo e construindo rampa de cores lógica..."):
    ...
    # --- PALETA HIPSOMETRIA (3 CLASSES - PROGRESSÃO LÓGICA) ---
    ranks_alt = pd.qcut(gdf['altitude'], 3, labels=[0, 1, 2]).astype(int)
    # Verde Escuro (Seguro) -> Laranja (Alerta) -> Roxo (Barreira)
    palette_3 = [[27, 94, 32, 180], [230, 126, 34, 180], [123, 31, 162, 180]]
    gdf['cor_altitude'] = [palette_3[r] for r in ranks_alt]

    # --- PALETA DECLIVIDADE (7 CLASSES - DEGRADÊ SEMAFÓRICO) ---
    ranks_slope = pd.qcut(gdf['declividade'].rank(method='first'), 7, labels=range(7)).astype(int)
    palette_7 = [
        [27, 94, 32, 180],    # 1. Verde Escuro (Plano)
        [76, 175, 80, 180],   # 2. Verde Padrão
        [139, 195, 74, 180],  # 3. Verde Lima
        [255, 235, 59, 180],  # 4. Amarelo (Atenção)
        [255, 152, 0, 180],   # 5. Laranja (Crítico NBR)
        [211, 47, 47, 180],   # 6. Vermelho (Muito Íngreme)
        [123, 31, 162, 180]   # 7. Roxo (Inacessível)
    ]
...
def renderizar_graficos(df_final):
    ...
    with col_a:
        ...
        fig1.update_traces(line_color='#7f8c8d', marker=dict(color='#1b5e20', size=6))
    ...
    with col_b:
        ...
        fig_dec.update_traces(line_color='#7b1fa2', marker=dict(color='#7b1fa2', size=6))
    ...
    fig3 = px.scatter(df_plot, x='altitude', y='Percentual de PCDs', color='Faixa de Relevo', size='Percentual de PCDs', hover_name='sub_bairro',
                      color_discrete_map={'1. Baixo': '#1b5e20', '2. Médio': '#e67e22', '3. Alto': '#7b1fa2'},
    ...
    fig4.add_trace(go.Scatter(
        ...
        marker=dict(size=24, color=['#1b5e20', '#e67e22', '#7b1fa2'], line=dict(width=2, color='white')),
